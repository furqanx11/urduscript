...
class Parser:
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Humen 'RETURN', 'CONTINUE', 'BREAK', 'banao', 'AGAR', 'FOR', 'JABTAK', 'FUNCTION', int, float, identifier, '+', '-', '(', '[' or 'NOT' ki Umeed thi",
                )
            )
    ...
    def expression(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "banao"):
            res.register_advancement()
            self.advance()
            ...
            if self.current_tok.type != TT_IDENTIFIER:
                return res.failure(
                    InvalidSyntaxError(
                        self.current_tok.pos_start,
                        self.current_tok.pos_end,
                        f"Humen 'Identifier' ki Umeed thi",
                    )
                )
        ...
        node = res.register(
            self.bin_op(self.comp_expression, ((TT_KEYWORD, "AUR"), (TT_KEYWORD, "YA")))
        )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Humen 'banaao', 'AGAR', 'FOR', 'JABTAK', 'FUNCTION', int, float, identifier, '+', '-', '(', '[' or 'NOT' ki Umeed thi",
                )
            )
    ...
    def comp_expression(self):
        ...
        node = res.register(
            self.bin_op(self.arith_expression, (TT_EE, TT_NE, TT_LT, TT_GT, TT_LTE, TT_GTE))
        )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Humen int, float, identifier, '+', '-', '(', '[', 'AGAR', 'FOR', 'JABTAK', 'FUNCTION' or 'NOT' ki Umeed thi",
                )
            )
                ...
                if res.error:
                    return res.failure(
                        InvalidSyntaxError(
                            self.current_tok.pos_start,
                            self.current_tok.pos_end,
                            "Humen ')', 'banao', 'AGAR', 'FOR', 'JABTAK', 'FUNCTION', int, float, identifier, '+', '-', '(', '[' or 'NOT' ki Umeed thi",
                        )
                    )
    ...
    def atom(self):
        ...
        return res.failure(
            InvalidSyntaxError(
                tok.pos_start,
                tok.pos_end,
                "Humen int, float, identifier, '+', '-', '(', '[', IF', 'FOR', 'WHILE', 'FWUNCTION' ki Umeed thi",
            )
        )
            ...
            if res.error:
                return res.failure(
                    InvalidSyntaxError(
                        self.current_tok.pos_start,
                        self.current_tok.pos_end,
                        "Humen ']', 'pwease', 'IF', 'FOR', 'WHILE', 'FWUNCTION', int, float, identifier, '+', '-', '(', '[' or 'NOT' ki Umeed thi",
                    )
                )
    ...
    def if_expression(self):
        res = ParseResult()
        all_cases = res.register(self.if_expression_cases("AGAR"))
        if res.error:
            return res
    ...
    def if_expression_b(self):
        return self.if_expression_cases("MAGAR")
    ...
    def if_expression_cases(self, case_keyword):
        ...
        if not self.current_tok.matches(TT_KEYWORD, case_keyword):
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    f"Humen '{case_keyword}' ki Umeed thi",
                )
            )
        ...
        if not self.current_tok.matches(TT_KEYWORD, "KARO"):
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    f"Humen 'KARO' ki Umeed thi",
                )
            )
        ...
        if isinstance(other, Number):
            ...
            try:
                ...
                return None, RTError(
                    other.pos_start,
                    other.pos_end,
                    "Element at this index cowuld not be retrieved fwom list because index is out of bounds",
                    self.context,
                )
        else:
            return None, Value.illegal_operation(self, other)
        ...
        if isinstance(other, Number):
            try:
                ...
                return None, RTError(
                    other.pos_start,
                    other.pos_end,
                    "Element at this index cowuld not be retrieved fwom list because index is ouwt of boundss",
                    self.context,
                )
        else:
            return None, Value.illegal_operation(self, other)
...
class Interpreter:
    ...
    def no_visit_method(self, node, context):
        raise Exception(f"ye {type(node).__name__} method defined nhi!!")
...
class IllegalCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(
            pos_start, pos_end, "Ayein!!, Bhai kia kar rahay ho? Illegal Character? ", details
        )
...
class ExpectedCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(
            pos_start, pos_end, "Ayein!!, Bhai kia kar rahay ho? Expected Character?  ", details
        )
...
class InvalidSyntaxError(Error):
    def __init__(self, pos_start, pos_end, details=""):
        super().__init__(
            pos_start, pos_end, "Ayein!!, Bhai kia kar rahay ho? Invalid Syntax? ", details
        )
...
class RTError(Error):
    def __init__(self, pos_start, pos_end, details, context):
        super().__init__(
            pos_start, pos_end, "Ayein!!, Bhai kia kar rahay ho? Runtime Error?  ", details
        )
        self.context = context
...
global_symbol_table.set("galat", Number.false)
global_symbol_table.set("sahi", Number.true)
global_symbol_table.set("mwath_pi", Number.math_PI)
global_symbol_table.set("dikhaao", BuiltInFunction.print)
global_symbol_table.set("print_ret", BuiltInFunction.print_ret)
global_symbol_table.set("bataao", BuiltInFunction.input)
global_symbol_table.set("bataao_int", BuiltInFunction.input_int)
global_symbol_table.set("clear", BuiltInFunction.clear)
global_symbol_table.set("cls", BuiltInFunction.clear)
global_symbol_table.set("num_hai", BuiltInFunction.is_number)
global_symbol_table.set("str_hai", BuiltInFunction.is_string)
global_symbol_table.set("list_hai", BuiltInFunction.is_list)
global_symbol_table.set("fun_hai", BuiltInFunction.is_function)
global_symbol_table.set("daalo", BuiltInFunction.append)
global_symbol_table.set("nikaalo", BuiltInFunction.pop)
global_symbol_table.set("bharao", BuiltInFunction.extend)
